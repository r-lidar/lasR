...
BOOL LASreaderASC::open(const CHAR* file_name, BOOL comma_not_point)
{
  ...
  snprintf(header.generating_software, 32, "via LASreaderASC (%d)", LAS_TOOLS_VERSION);
  ...
  if (!complete)
  {
    eprint("ERROR: was not able to find header\n");
    return FALSE;
  }
  ...
  {
    eprint("WARNING: ASC raster contains only no data values\n");
    header.min_z = 0;
    header.max_z = 0;
  }
