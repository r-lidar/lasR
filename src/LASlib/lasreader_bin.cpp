...
BOOL LASreaderBIN::open(ByteStreamIn* stream)
{
  ...
  {
    eprint("ERROR: reading terrasolid header\n");
    return FALSE;
  }
  ...
  {
    eprint("ERROR: corrupt terrasolid header. size != 56.\n");
    return FALSE;
  }

  if (tsheader.recog_val != 970401)
  {
    eprint("ERROR: corrupt terrasolid header. recog_val != 979401.\n");
    return FALSE;
  }

  if (strncmp(tsheader.recog_str, "CXYZ", 4) != 0)
  {
    eprint("ERROR: corrupt terrasolid header. recog_str != CXYZ.\n");
    return FALSE;
  }
...
BOOL LASreaderBIN::read_point_default()
{
  if (p_count < npoints)
  {
    ...
    if (version == 20020715)
    {
      ...
      {
        eprint("ERROR: reading terrasolid point after %u of %u\n", (U32)p_count, (U32)npoints);
        return FALSE;
      }
    ...
    else
    {
      ...
      {
        eprint("ERROR: reading terrasolid row after %u of %u\n", (U32)p_count, (U32)npoints);
        return FALSE;
      }
      point.set_X(tsrow.x);
      point.set_Y(tsrow.y);
    ...
    if (point.have_gps_time)
    {
      ...
      {
        eprint("ERROR: reading terrasolid time\n");
        return FALSE;
      }
    ...
    if (point.have_rgb)
    {
      ...
      {
        eprint("ERROR: reading terrasolid color\n");
        return FALSE;
      }
