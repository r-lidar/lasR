...
public:
  ...
  BOOL write_point(const LASpoint* point) override;
  BOOL chunk() override;

  BOOL update_header(const LASheader* header, BOOL use_inventory=FALSE, BOOL update_extra_bytes=FALSE) override;
  I64 close(BOOL update_npoints=TRUE) override;
