...
// these functions set the desired point format (and maybe add on attributes in extra bytes)
  ...
  // LASR addition
  BOOL inside_buffer(const F64 min_x, const F64 min_y, const F64 max_x, const F64 max_y, const BOOL circle = FALSE)
  {
    F64 x = get_x();
    ...
    if (circle)
    {
      F64 r2 = (max_x-min_x)/2;
      F64 center_x = (max_x+min_x)/2;
      F64 center_y = (max_y+min_y)/2;
      F64 dx = center_x - get_x();
      F64 dy = center_y - get_y();
