...
attribute does not exist `NaN` is returned. Thus `intesity_mean` (with
typo) return `NaN` rather than failing. Last the metric names supports
modifiers like absolute value. Thus `|a|_mean`, `|angle|_mean`,
`|ScanAngle|_mean` are valid operators.
...
names of the attributes. Be careful of typos. The attributes existence
