...
test_that("Max depth in COPC",
{
  ...
  pipeline = reader_las(filter = "-max_depth 0") + summarise()
  ans = exec(pipeline, on = f)
  expect_equal(ans$npoints, 61201L)

  pipeline = reader_las(filter = "-max_depth 1") + summarise()
  ans = exec(pipeline, on = f)
...
test_that("Write COPC",
{
  ...
  pipeline = reader_las(filter = "-max_depth 2") + write_las(o)
  ans = exec(pipeline, on = f)
  ...
  pipeline = reader_las(filter = "-max_depth 1") + summarise()
  ans = exec(pipeline, on = o)
  expect_equal(ans$npoints, 400000L,tolerance = 0.0001)

  pipeline = reader_las(filter = "-max_depth 2") + summarise()
  ans = exec(pipeline, on = o)
