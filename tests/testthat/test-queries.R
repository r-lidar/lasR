...
{
  pipeline = reader_las_circles(885100, 629300, 10) + read_las()
  ans = exec(pipeline, f)
  ...
  pipeline = reader_las_circles(885150, 629400, 10, buffer = 5L) + read_las()
  ans = exec(pipeline, f)
  ...
  pipeline = reader_las_rectangles(885000L, 629390, 885040, 629410, buffer = 5) + read_las()
  ans = exec(pipeline, f)
  expect_equal(dim(ans), c(5028L, 5L))
  expect_equal(sum(ans$Buffer), 2415L)

  # no match
  pipeline = reader_las_circles(8850000, 629400, 20, buffer = 5) + read_las()
  expect_error(exec(pipeline, f), "cannot find")
})
