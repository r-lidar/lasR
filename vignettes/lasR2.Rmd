...
read <- reader_circles(f, xcenter, ycenter, 11.28) 
tri <- triangulate(filter = keep_ground())
trans <- transform_with_triangulation(tri)
norm <- tri + trans
metric <- callback(my_metric_fun, expose = "z", drop_buffer = TRUE)
...
pipeline = reader_circles(f, x, y, 20) + rasterize(2, "max", ofile = ofile)
...
While `lasR` does not depends on `lidR` it has some compatibilities with it. Instead of providing paths to files or folder it is possible to pass a `LAScatalog` to the readers.
