...
The `reader` algorithm must be the first algorithm of any pipeline. It takes as input the list of files to process. When creating a pipeline with only this algorithm, the files are read, but no computation is actually applied. The point are not even read. Files are read sequentially, with points loaded one at a time and immediately discarded. No result is returned.
...
```
Using `triangulate()` without any other algorithm in the pipeline is usually not very useful. Typically, `triangulate()` is employed without the `ofile` argument as an intermediate step. For instance, it can be used with `rasterize()`.
...
After performing normalization, users may want to write the normalized point cloud to disk for later use. In this case, you can append the `write_las()` algorithm to the pipeline.
...
As observed, `callback` does not explicitly return anything; however, it edits the point cloud without producing a visible result. To generate an output, users must use `write_las()`. It's important to note that `write_las()` will **NOT** write the point number 13 which is flagged `withheld`.
...
In conclusion, in the hypothesis that the user-defined function returns something complex, there are two ways to handle the buffer: either using the bounding box or using the `Buffer` flag.

## Boundaries

A Delaunay triangulation defines a convex polygon, which represents the concave hull of the points. However, in dense point clouds, removing triangles with large edges due to the absence of points results in a more complex structure.

```{r, echo = FALSE}
f <- system.file("extdata", "Topography.las", package="lasR")
```

```{r boundaries, fig.width=5, fig.height=5}
read = reader(f)
...
The `boundaries()` algorithm computes the contour of the mesh, producing a convex hull with holes:

```{r boundaries2, fig.width=5, fig.height=5}
read = reader(f)
del = triangulate(15, filter = keep_ground())
bound = boundaries(del)
...
However `boundaries()` is more likely to be used without a triangulation. In this case it returns the bounding box of each LAS/LAZ file read from the header.
...
- `reader_file_coverage()`: will read of the files and process the entire point cloud.
- `reader_file_rectangles()`: will read only some rectangular regions of interest of the coverage and process then sequentially.
- `reader_file_circles()`: will read only some circular regions of interest of the coverage and process then sequentially.
- `reader_dataframe_coverage()` : will read a `data.frame` and convert it internally into the internal `lasR`'s memory structure. It processes the entire point-cloud.
- `reader_dataframe_rectangles()`: will read a `data.frame` and convert it internally into the internal `lasR`'s memory structure. It processes only the regions of interest sequentially.
- `reader_dataframe_circles()`: will read a `data.frame` and convert it internally into the internal `lasR`'s memory structure. It processes only the regions of interest sequentially.
