...
`lasR` does not parse R expression such as `ReturnNumber != x` thus on must use `LASlib` filters. Each stage of the pipeline has it own filter. See `?lasR::filters`.
...
There is currently not implementation but this will be added later
...
{
  read = reader_circles(f, xcenter, ycenter, radius)
