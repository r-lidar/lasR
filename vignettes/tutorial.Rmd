...
The `reader_las()` stage MUST be the first stage of any pipeline (blue in the figure above). This stage reads the point cloud if necessary. When creating a pipeline with only this stage, the header of the files are read, but no computation is actually applied. The points are thus not even read because there is no other stage in the pipeline that require to read the points. No result is returned.
...
A pipeline can be a long succession of stages and it may happens that we do not want to apply the entire pipeline on every files. In this case the `stop_if` stages allows to conditionally stop the pipeline anywhere. Let assume we have a dataset with 100 files and pipeline that reads, computes the hulls, computes the DTM
...
It is possible to computes the hulls on the 100 files but the DTM only in a reduced regions of interest defined by the user with `stop_if_outside()`
...
`stop_if` is (currently) the only stage than can be put before `reader_las()`. In this case the reading stage is skipped, effectively applying the pipeline on a subset of the files than encompasse the bounding boxe defined by the user.

```r
pipeline <- stopif + read + hll + tri + dtm
```

Notice that the pipeline below, produces the same output than the pipeline above but takes longer to compute because the files that are not processed are read anyway. It is thus preferable to put `stop_if()` before `reader_las()`in this specific case.

```r
pipeline <- read + stopif + hll + tri + dtm
```

Currently there is only one `stop_if` stage that conditionally stops the pipeline based on bounding box condition but if will be easy to add more options later on demand.
