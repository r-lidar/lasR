...
This vignette shows a list of pipeline factories, i.e., functions that generate useful pieces of pipelines that can be reused. It also defines some functions that execute a pipeline, which may be regularly useful instead of writing it out. The lists are not comprehensive and will be updated if good tools come to mind.
...
{
  pipeline = triangulate(filter = keep_ground())
...
clip_circle = function(files, geometry, radius, ofiles = paste0(tempdir(), "/*_clipped.las"))
{
  ...
  read = reader_circles(files, xcenter, ycenter, radius)
...
A CRS as `sf` object. The cost of applying `boundaries()` is virtually null.

```{r}
crs = function(files)
{
  pipeline = reader(files) + boundaries()
...
```{r}
inventory_metrics = function(geometry, radius, fun, normalize = FALSE)
{
  if (sf::st_geometry_type(geometry, FALSE) != "POINT") 
    stop("Expected POINT geometry type")
  ...
  pipeline <- reader_circles(f, xcenter, ycenter, radius)
...
inventory_metrics(geometry, radius, fun)
```
