...
# AOI:       10,000 acres (6,361 m × 6,361 m), same as the lasR benchmark
# Modes:
#   A. PDAL single-process with `readers.ept` `threads=N` (N in 1, 2, 4, 8, 16)
#   B. PDAL 16 concurrent processes, AOI split into a 4×4 grid (threads=1 each)
#
...
XMIN=-12416511.5;  YMIN=3755441.5
XMAX=-12410150.5;  YMAX=3761802.5

WORK=$(mktemp -d /tmp/pdal_bench.XXXXXX)
trap "rm -rf '$WORK'" EXIT

mk_pipeline() {
  # $1=xmin $2=xmax $3=ymin $4=ymax $5=requests $6=outfile
  # NOTE: PDAL's readers.ept uses `bounds` as "([xmin, xmax], [ymin, ymax])"
  # and the parallel-fetch knob is `requests`, not `threads`.
  cat > "$6" <<EOF
{
  "pipeline": [
    {
...
run_one() {
  ...
  local t0 t1
  t0=$(date +%s.%N)
  # capture metadata JSON; count = filters.stats.statistic[0].count
  ...
  # filters.stats output: stages."filters.stats".statistic[].count is point count per dim.
  local npts
  npts=$(python3 - "$pipeline.meta.json" <<'PY'
import json, sys
m = json.load(open(sys.argv[1]))
st = m.get("stages",{}).get("filters.stats",{}).get("statistic",[])
# Each dimension reports its own count. They should all be equal; take the first.
...
echo
# --- B. 16 concurrent processes, 4x4 AOI grid, threads=1 each ----
...
# Build per-tile pipelines first
...
# Correctness — they should all return the same total
