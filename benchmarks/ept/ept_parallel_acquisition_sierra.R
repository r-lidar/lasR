...
#
# Endpoint:  USGS 3DEP CA_SierraNevada_12_B22 (~80.5B points, native EPSG:3857).
#            ~5x point density of AZ_BrawleyRillito_TL_2018, so AOI is shrunk
#            to keep workload (~60M points) comparable to the AZ benchmark.
# AOI:       2,049 acres (2,880 m x 2,880 m), centered inside the
...
suppressMessages(library(lasR, lib.loc = "/home/alex/projects/lasR/.Rlib"))

stopifnot(has_omp_support())

URL <- "https://s3-us-west-2.amazonaws.com/usgs-lidar-public/CA_SierraNevada_12_B22/ept.json"

# boundsConforming: [-13428080, 4454031, -13355765, 4611240]
# Dataset density ~7.1 Mpts/km² → 2,049 acres ≈ 8.29 km² gives ~58M points,
# matching the AZ benchmark workload. Side 2,880 m → half 1,440 m.
cx <- (-13428080 + -13355765) / 2   # -13391922
cy <- ( 4454031 +  4611240) / 2     #   4532635
h  <- 1440.0
xmin <- cx - h; xmax <- cx + h
ymin <- cy - h; ymax <- cy + h
